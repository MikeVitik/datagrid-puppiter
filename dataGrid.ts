...
export default class DataGrid {
  ...
  getGridInstance: () => Promise<JSHandle<{}>>;
 ...
 constructor(id: string, name = 'dxDataGrid') {
    //super(id);
    this.element = page.waitForSelector(id);

    this.name = name;
    this.dataRows = this.element
      .then(DataGrid.waitForChild(`.${CLASS.dataRow}`));

    // const grid = this.element;

    this.getGridInstance = () => page.evaluateHandle((name) => ($(id) as any)[name]('instance'), name);
    /*ClientFunction(
      () => $(grid())[`${name}`]('instance'),
      { dependencies: { grid, name } },
    );*/
  }
  static waitForChild = (selector: string) => (el: ElementHandle)  => {
    return page.waitForFunction((el: Element, selector) => {
      console.log(el, selector, el.querySelector<Element>(selector));
      return el.querySelector<Element>(selector);
    }, {}, el, selector)
    .then(h => h.asElement() as ElementHandle);
  }
  ...
  addWidgetPrefix(className: string) {
    return DataGrid.addClassPrefix(this.name, className);
  }
...
*/
  getDataRow(index: number): DataRow {
    return new DataRow(this.element.then(DataGrid.waitForChild(`.${CLASS.dataRow}[aria-rowindex='${index + 1}']`)), this.name);
  }

  getFixedDataRow(index: number): DataRow {
    return new DataRow(this.element
      .then(DataGrid.waitForChild(`.${this.addWidgetPrefix(CLASS.fixedGridView)} .${CLASS.dataRow}[aria-rowindex='${index + 1}']`)),
       this.name
      );
  }
